...
def run_gpt_generate_importance(
  ...
  def _func_clean_up(gpt_response, prompt=""): 
    # Store original response for error reporting
    original_response = gpt_response
    
    # Check if response is None or empty
    if gpt_response is None:
      raise RuntimeError(f"LLM returned None response. This indicates a connection or generation error.")
    
    # Ensure response is a string
    if not isinstance(gpt_response, str):
      raise TypeError(f"Expected string response from LLM, got {type(gpt_response).__name__}: {gpt_response}")
    
    # Check if response is empty
    if not gpt_response.strip():
      raise RuntimeError(f"LLM returned empty response. This indicates a generation error.")
    
    # Check if we got the fail_safe value (indicates all retries failed)
    if gpt_response == "25" or "GENERATION ERROR" in gpt_response:
      raise RuntimeError(f"Failed to generate importance scores after 3 retries. Last error: {gpt_response}")
    
    # Try to extract JSON from response
    parsed_response = extract_first_json_dict(gpt_response)
    if parsed_response is None:
      ...
      raise ValueError(
        f"Failed to parse importance scores from LLM response. "
        f"The LLM response could not be parsed as JSON. "
        f"Raw response (first 500 chars): {response_preview}"
      )
    
    scores = list(parsed_response.values())
    if len(scores) != len(records):
      response_preview = gpt_response[:500] if len(gpt_response) > 500 else gpt_response
      raise ValueError(
        f"Mismatch in importance scores: expected {len(records)} scores, got {len(scores)}. "
...
def extract_relevance(seq_nodes, embeddings, focal_pt): 
  ...
  focal_dim = len(focal_embedding)

  relevance_out = dict()
  for count, node in enumerate(seq_nodes): 
    # Skip nodes with incompatible embedding dimensions
    if node.content not in embeddings:
      # If embedding is missing, assign neutral relevance score
      relevance_out[node.node_id] = 0.0
      continue
    
    node_embedding = embeddings[node.content]
    
    # Skip embeddings with dimension mismatch (e.g., old OpenAI 1536 vs new Ollama 768)
    if not isinstance(node_embedding, list) or len(node_embedding) != focal_dim:
      # Assign neutral relevance score for incompatible embeddings
      relevance_out[node.node_id] = 0.0
      continue
...
class MemoryStream: 
  def __init__(self, nodes, embeddings): 
    ...
    # Store embeddings as-is; dimension mismatches will be handled gracefully in extract_relevance
  ...
  def remember(self, content, time_step=0):
    # Generate importance score - will retry up to 3 times via chat_safe_generate
    # If it fails after retries, exception will be raised (no fallbacks)
    scores = generate_importance_score([content])
    
    # Validate we got exactly one score
    if not scores or len(scores) != 1:
      raise ValueError(f"Expected 1 importance score, got {len(scores) if scores else 0}")
  ...
  def reflect(self, anchor, reflection_count=5, 
    ...
    # Generate importance scores - will retry up to 3 times via chat_safe_generate
    # If it fails after retries, exception will be raised (no fallbacks)
    scores = generate_importance_score(reflections)
    
    # Validate scores match reflections count
    if not scores or len(scores) != len(reflections):
      raise ValueError(f"Importance score count mismatch: expected {len(reflections)} scores, got {len(scores) if scores else 0}")
